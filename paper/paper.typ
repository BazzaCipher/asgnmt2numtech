...
The two DCC fits behave very differently. The standard DCC fits at $(a, b) = (0.005, 0.893)$ with persistence $a + b = 0.898$, putting weight $1 - a - b = 0.102$ on the long-run target $overline(R)$ at each step. The MP-cleaned DCC fits at the cap: $(a, b) = (0.003, 0.995)$, $a + b = 0.999$ (the stationarity boundary), weight on the cleaned target $tilde(R)$ of just $0.0015$ -- the QMLE, when forced to use the cleaned target, drives the weight on that target down to *0.15%*, sixty-eight times lower than the standard DCC, and the optimiser fails to converge to an interior solution. The log-likelihood of the cleaned model is *529 nats lower* than the standard model's, at identical parameter count. Neither test of model fit favours the cleaned target.
...
For S3, on the cocoa NY × London basis under the cleaned DCC, the baseline 2019--2023 mean is 0.551 and the *crisis (2024--2026, full window) mean is 0.663* — a *rise* of 0.112, not a drop. The crisis minimum is 0.544, only 0.007 below the baseline mean. *S3 fails in the predicted direction*; the result is in fact the opposite of the prediction. We discuss the interpretation in #link(<sec:disc>)[§6].
...
@fig:robust runs the rolling-count test under four variants alongside the baseline: (i) excluding GHANA10Y entirely (addresses the universe-size-change concern); (ii) a 504-day window; (iii) (i)+(ii) combined; (iv) GARCH refit with Gaussian innovations. The +1 median lift on T2(b) holds under variants (i) and (iv) at W=252. Under W=504 the medians saturate at 3 in both pre- and in-crisis windows, so the bootstrap test cannot distinguish them; BIC-selected Bai--Perron on the longer window detects its first crisis break in *2025-04-02*, outside the pre-registered crisis-attribution window $[2024"-"01"-"01, 2024"-"12"-"31]$, so T2(a) fails on this variant. Honest summary: the +1 count lift is real and survives most perturbations, but neither component of T2 survives the W=504 robustness check on its pre-registered terms.

= Discussion <sec:disc>

Three findings warrant discussion.

*The +1 count lift is real but does not pass either pre-registered T2 test on its terms.* The rolling 252-day supra-MP count rises from a median of 2 in the pre-crisis baseline to 3 in the crisis, a difference that is visible in the data and survives the most plausible robustness variants (dropping GHANA10Y, switching to Gaussian innovations). However, neither component of T2 *as pre-registered* survives: T2(a) under BIC-selected Bai--Perron returns zero breaks; T2(b) is a one-bit comparison on an integer count, with the bootstrap distribution literally degenerate. The honest reading is that the +1 lift is consistent with a structural break but is not statistically *confirmed* as one. The 2024-07-08 break date that earlier circulated was an artefact of forcing five breaks; it does not survive BIC selection.
...
- `R/diagnostics.R`, `R/diagnostics_extra.R` (end-to-end numerical diagnostics; re-run to verify every figure quoted in this paper)
