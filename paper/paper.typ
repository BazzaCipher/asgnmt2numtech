...
- *Cocoa-bloc concentration.* An eigenvalue that crosses from below the MP edge in pre-crisis to above it in crisis, with eigenvector loading $gt.eq 0.50$ of its squared mass on the cocoa bloc $\{$COCOA_NY, COCOA_LDN, HSY, MDLZ, LISN, GHS, GHANA10Y$\}$.
...
= Appendix A: Threshold parameterisation for the cocoa-bloc concentration test <sec:appendix>

The cocoa-bloc concentration test (T3) is pre-registered with a threshold of 0.50 on the rank-1 supra-MP eigenvector's cocoa-bloc squared-loading share -- the majority-rule cutoff under which the bloc accounts for more than half of the eigenvector's squared mass. An alternative parameterisation, 0.55, ties the threshold to roughly twice the bloc's proportional share of the universe ($7\/25 approx 0.28$; $2 times approx 0.56$, rounded to 0.55), demanding the bloc be over-represented by a factor of two relative to its weight. The observed crisis-subsample rank-1 share of 0.516 clears the 0.50 threshold by 0.016 and falls 0.034 short of 0.55. The doubling-relative-to-pre-crisis leg of T3 (the rank-1 share rising from 0.251 to 0.516, a 2.05$times$ increase) is independent of the threshold level and is satisfied by a narrow margin under either parameterisation.
